...
type
  ...
  protected
    ...
    // EZ
    property UseEditWndProc: Boolean read FUseEditWndProc write FUseEditWndProc;
...
begin
  ...
  try
    FStopping := True;
    if Assigned(FEdit) then begin // EZ
      FEdit.Hide;
...
var
  Sep: Char;
begin
  Sep := SysUtils.FormatSettings.DateSeparator;
  try
    if FDateSeparator <> #0 then SysUtils.FormatSettings.DateSeparator := FDateSeparator;
      if FDateFormat <> '' then
    ...
    SysUtils.FormatSettings.DateSeparator := Sep;
  end;
end;
